...
class Inventory(EasyFrame):
    ...
    def __init__(self):
        ...
        self.upload_btn = self.addButton(text="Upload Excel File", row=5, column=0, command=self.upload_excel)
        self.upload_btn.grid(row=5, column=1, sticky="EW", padx=10, pady=10)
    ...
    def upload_excel(self):
        ...
        try:
            ...
            """Converts DataFrame into Dictionary List"""
            self.employees = emp_df.to_dict(orient='records')
            messagebox.showinfo("Success", "Successfully uploaded!")
        except Exception as e:
            messagebox.showerror("Error", f"Could not upload file: {e}")
    ...
    def open_employee_window(self):
        ...
        Label(popup, text="Employee Management", bg="beige", fg="navajowhite3", font=("Arial", 16, "bold")).grid(
            row=0, column=0, columnspan=2, sticky="EW", pady=0)
    ...
    def add_employee(self):
        ...
        if not re.match(r"^[A-Za-z\s]+$", name):
            messagebox.showerror("Invalid Name", "Name must contain only letters and numbers.")
            return

        if not re.match(r"^[A-Za-z0-9]+$", emp_id):
            messagebox.showerror("Invalid ID", "ID must contain only letters and numbers.")
            return

        if any (emp["id"] == emp_id for emp in self.employees):
            messagebox.showerror("Employee Already Exists", f"Employee ID {emp_id} already exists.")

        if len(name) > 20:
            messagebox.showerror("Employee Name Too Long", "Employee name must be less than 20 characters.")
            return

        if len(emp_id) > 6:
            messagebox.showerror("Employee ID Too Long", "Employee ID must be less than 6 characters.")
            return
